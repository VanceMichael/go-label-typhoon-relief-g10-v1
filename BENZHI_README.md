...
./build_benzhi_docker.sh benzhi-task-384-amd64 linux/amd64
./build_benzhi_docker.sh benzhi-task-384-arm64 linux/arm64
docker run -it benzhi-task-384-amd64:latest
docker run -it --platform linux/arm64 benzhi-task-384-arm64:latest
```

## 题目验证命令

1. 预期退出码 0：`go test ./internal/integration -run '^TestShelterReserveRollsBackWhenAuditFails$' -count=1`
2. 预期退出码 0：`go test ./...`
