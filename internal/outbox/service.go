...
func (s *Service) Claim(ctx context.Context, owner string, lease time.Duration, now time.Time) (domain.OutboxEvent, error) {
	...
	err := s.DB.Tx(ctx, func(tx *sql.Tx) error {
		...
		res, err := tx.ExecContext(ctx, `UPDATE outbox_events SET lease_owner=?,lease_until=?,attempts=attempts+1 WHERE id=? AND status='pending' AND (lease_until IS NULL OR lease_until<?)`, owner, sqlite.FormatTime(ev.LeaseUntil), ev.ID, sqlite.FormatTime(now))
		if err != nil {
			return err
		}
